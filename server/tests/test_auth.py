...
def test__login__given_credentials__when_already_authenticated__then_400_returned(client):
    access_token = client.post("/api/auth/login", json={"username": "admin", "password": "xss"}).json["access_token"]
...
def test__login__given_credentials__when_password_is_wrong__then_403_returned(client):
...
def test__login__given_credentials__when_credentials_are_valid__then_200_returned(client):
...
def test__refresh__given_refresh_token__then_new_token_returned(client):
    refresh_token = client.post("/api/auth/login", json={"username": "admin", "password": "xss"}).json["refresh_token"]
    response = client.post("/api/auth/refresh", headers={"Authorization": f"Bearer {refresh_token}"})
    assert "access_token" in response.json
    assert response.status_code == 200


def test__logout__given_jti__then_added_to_blocklist(client):
    refresh_token = client.post("/api/auth/login", json={"username": "admin", "password": "xss"}).json["refresh_token"]
