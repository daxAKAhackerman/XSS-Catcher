...
def test_xss_to_dict(client):
    ...
    create_client(client, name='name1', description='desc1')
    client_name1 = Client.query.first()
    post_x(client, 'r', client_name1.uid, cookies='cookie=good',
           local_storage='{"local":"good"}', session_storage='{"session":"good"}', param='good', fingerprint='["good"]', dom='<br />', screenshot='O==')
