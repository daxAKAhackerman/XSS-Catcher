...
def test_patch_settings(client):
    ...
    patch_settings(client, smtp_host="127.0.0.1", smtp_port=465, ssl_tls=True, mail_from="xsscatcher@hackerman.ca", smtp_user="admin", smtp_pass="admin")
    settings = Settings.query.first()
    assert settings.smtp_host == "127.0.0.1"
    rv = patch_settings(client, smtp_host="{}.test.com".format("a" * 256), smtp_port=465)
    assert b"Server address too long" in rv.data
    rv = patch_settings(client, smtp_host="127.0.0.1", smtp_port="a")
    assert b"Port is invalid" in rv.data
    rv = patch_settings(client, smtp_host="127.0.0.1", smtp_port=65536)
    assert b"Port is invalid" in rv.data
    rv = patch_settings(client, smtp_host="127.0.0.1")
    assert b"Missing SMTP port" in rv.data
    rv = patch_settings(client, smtp_host="127.0.0.1", smtp_port=465, starttls=True, ssl_tls=True)
    assert b"Cannot use STARTTLS and SSL/TLS at the same time" in rv.data
    patch_settings(client, smtp_host="127.0.0.1", smtp_port=587, starttls=True, mail_from="xsscatcher@hackerman.ca")
    settings = Settings.query.first()
    ...
    rv = patch_settings(client, smtp_host="127.0.0.1", smtp_port=25, mail_from="test")
    assert b"Email address format is invalid" in rv.data
    rv = patch_settings(client, smtp_host="127.0.0.1", smtp_port=25)
    assert b"Missing sender address" in rv.data
    rv = patch_settings(client, smtp_host="127.0.0.1", smtp_port=25, mail_from="xsscatcher@hackerman.ca", smtp_user="a" * 129, smtp_pass="admin")
    assert b"SMTP username too long" in rv.data
    rv = patch_settings(client, smtp_host="127.0.0.1", smtp_port=25, mail_from="xsscatcher@hackerman.ca", smtp_user="admin", smtp_pass="a" * 129)
    assert b"SMTP password too long" in rv.data
    patch_settings(client, smtp_host="")
    settings = Settings.query.first()
...
def test_send_mail(client):
    ...
    patch_settings(client, smtp_host="127.0.0.1", smtp_port=587, ssl_tls=True, mail_from="xsscatcher@hackerman.ca")
    rv = send_test_mail(client, mail_to="dax@hackerman.ca")
    assert b"Could not send test email" in rv.data
    try:
        send_mail()
